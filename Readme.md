## Description
CustomPOS is a retail Point of sale system programed with LibreOffice macro so that you can customize it easily. Don't think it is a simple spreadsheet file, it has network multi-user capabilities by using proven database engine. The interface has been designed for the best possible efficiency with reduced typing needs, POS module is made of one page only where invoices are recorded after one popup validation only.
